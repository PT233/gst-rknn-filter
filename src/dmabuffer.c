...
int dmabuf_heap_open()
{
	...
	/* Try multiple heap paths (kernel/board dependent) */
	...
	for (int i = 0; heap_names[i] != NULL; i++) {
		int fd = open(heap_names[i], O_RDWR, 0);
		if (fd >= 0) {
			printf("%s: opened %s, fd=%d\n", __func__, heap_names[i], fd);
			dma_heap_fd = fd;
			dma_buf_opened = 1;
			return fd;
		}
	}
	printf("%s: failed to open dma_heap (tried cma@0, cma, linux,cma, system)\n", __func__);
	return -1;
}
